...
for year in 2015; do
    ...
    files0=( /jet/home/xjliu/Amazon_exp/Amazon_NoahMP/3Dfields_d01_$year-11-* )
    files1=( /jet/home/xjliu/Amazon_exp/Amazon_NoahMP_ILdeforested/3Dfields_d01_$year-11* )
   
   ### concatenate the hourly data of the same day
#      echo ${files0[@]}
      echo ${files1[@]}
        fout0=$outdir/$varname.${year}11.NoahMP.obs
        fout1=$outdir/$varname.${year}11.NoahMP.ILgrassland.
    ncrcat -O -v $var ${files0[@]} $fout0.nc
    ncrcat -O -v $var ${files1[@]} $fout1.nc
	ncea -O -v $var ${files0[@]:5:25} $fout0.multidaymean.nc
